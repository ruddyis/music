...
export default {
    ...
    this.$refs.scrollView.scrolling((y) => {
      ...
      for (let i = 0; i < this.groupsTop.length - 1; i++) {
        ...
        if (-y >= preTop && -y <= nextTop) {
          ...
          const diffOffsetY = nextTop + y // 当前滚动的偏移位y是负数
  ...
  watch: {
    list () {
      // console.log(this.$refs.group) //undefined,因为当list发生变化时,数据还没加载完毕,所以不能获取
